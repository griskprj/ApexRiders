...
class Member(db.Model):
    ...
    manual_progress = db.relationship(
        'UserManualProgress', back_populates='user', lazy=True)
    manual_ratings = db.relationship(
        'ManualRating', back_populates='user', lazy=True)
...
class Motorcycle(db.Model):
    ...
    member = db.relationship(
        'Member', backref=db.backref('motorcycles', lazy=True))
...
class MotorcycleNote(db.Model):
    ...
    # maintenance, repair, modification, general
    ...
    motorcycle = db.relationship(
        'Motorcycle', backref=db.backref('notes', lazy=True))
...
class Like(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'target_type',
                            'target_id', name='unique_like'),
    )
...
class MotorcycleMaintenance(db.Model):
    ...
    motorcycle = db.relationship(
        'Motorcycle', backref=db.backref('maintenance_tasks', lazy=True))
...
class MaintenanceManual(db.Model):
    ...
    author_id = db.Column(db.Integer, db.ForeignKey(
        'members.id'), nullable=False)
    author = db.relationship('Member', backref=db.backref(
        'authored_manuals', lazy=True), foreign_keys=[author_id])
    ...
    steps = db.relationship('ManualStep', back_populates='manual', lazy=True,
                            order_by='ManualStep.order', cascade='all, delete-orphan')
    ratings = db.relationship(
        'ManualRating', back_populates='manual', lazy=True)
    progress_records = db.relationship(
        'UserManualProgress', back_populates='manual', lazy=True)
...
class ManualDraft(db.Model):
    ...
    last_modified = db.Column(db.DateTime, default=lambda: datetime.now(
        timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
    created_at = db.Column(
        db.DateTime, default=lambda: datetime.now(timezone.utc))
...
class ManualStep(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    manual_id = db.Column(db.Integer, db.ForeignKey(
        'manuals.id'), nullable=False)
    order = db.Column(db.Integer, nullable=False)
    title = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text, nullable=False)
...
class UserManualProgress(db.Model):
    ...
    started_at = db.Column(
        db.DateTime, default=lambda: datetime.now(timezone.utc))
    last_activity = db.Column(db.DateTime, default=lambda: datetime.now(
        timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
    total_time_spent = db.Column(db.Integer, default=0)

    __table_args__ = (
        db.UniqueConstraint('user_id', 'manual_id',
                            name='unique_user_manual_progress'),
    )

    user = db.relationship('Member', back_populates='manual_progress')
    manual = db.relationship(
        'MaintenanceManual', back_populates='progress_records')
...
class ManualRating(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'manual_id',
                            name='unique_user_manual_rating'),
    )
...
class Notification(db.Model):
    ...
    # like_post, like_comment, like_product, admin_report, admin_broadcast
    ...
    user = db.relationship('Member', backref=db.backref(
        'notifications', lazy=True), foreign_keys=[user_id])
