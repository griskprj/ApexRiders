        ...
        if last_date_str:
            try:
                last_maintenance_date = datetime.fromisoformat(
                    last_date_str.replace('Z', '+00:00')).date()
            except ValueError:
                last_maintenance_date = datetime.strptime(
                    last_date_str, '%Y-%m-%d').date()
        else:
...
def update_maintenance(task_id):
    ...
    try:
        ...
        if 'last_maintenance_date' in data:
            try:
                if data['last_maintenance_date']:
                    task.last_maintenance_date = datetime.strptime(
                        data['last_maintenance_date'], '%Y-%m-%d').date()
            except (ValueError, TypeError):
                pass
        ...
        if task.schedule_type == 'mileage' and task.interval_value and task.last_maintenance_mileage:
            ...
            task.next_maintenance_date = task.last_maintenance_date + \
                timedelta(days=days)
        ...
        if task.is_recurring and data.get('create_next', True):
            ...
            if task.schedule_type == 'mileage':
                ...
                if task.interval_value == 'months':
                    next_date = next_date + \
                        timedelta(days=task.interval_value * 30)
